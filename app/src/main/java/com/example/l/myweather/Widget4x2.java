...
public class Widget4x2 extends AppWidgetProvider{
    ...
    /*public  void setViewShow(int[] appWidgetIds){

        week_ = getWeek(calendar.get(Calendar.DAY_OF_WEEK));
        initCity();

        for (int id : appWidgetIds){
            RemoteViews remoteViews = new RemoteViews(context.getPackageName(),R.layout.widget4x2_layout);
            int height = appWidgetManager.getAppWidgetOptions(id).getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_HEIGHT);
            if (height <= 100){
                remoteViews.setViewVisibility(R.id.forecast_layout,View.GONE);
                remoteViews.setViewVisibility(R.id.line, View.GONE);
                remoteViews.setTextViewTextSize(R.id.widget_time, TypedValue.COMPLEX_UNIT_DIP, 35);
                remoteViews.setViewVisibility(R.id.refresh_layout,View.GONE);
                remoteViews.setTextViewText(R.id.date,city + "  " + month + "/" + day  + "   " + week_);
            } else{
